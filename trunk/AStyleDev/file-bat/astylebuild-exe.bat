@echo off
REM build the AStyle Release Configuration using the AStyle project file
...
if not exist astylebuild-exe.bat (
set called=yes
set astyle=..\..\%astyle%
set bindir=..\..\%bindir%
)

echo BUILDING ASTYLE EXE
%windir%\microsoft.net\framework\v3.5\msbuild  /nologo  "/property:Configuration=Release"  "%astyle%\AStyle.sln"
...
xcopy "%astyle%\bin\AStyle.exe"  "%bindir%\"  /y /q
if %ERRORLEVEL% NEQ 0  set errors=yes
)
