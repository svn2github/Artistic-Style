...
%windir%\microsoft.net\framework\v3.5\msbuild  /nologo  "/property:Configuration=Debug Dll"  "%astyle%\AStyle.sln"
...
xcopy "%astyle%\bin\AStyled.dll"  "%bindir%\"  /y /q
if %ERRORLEVEL% NEQ 0  set errors=yes
xcopy "%astyle%\bin\AStyled.lib"  "%bindir%\"  /y /q
if %ERRORLEVEL% NEQ 0  set errors=yes
)
