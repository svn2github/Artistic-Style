@echo off
REM build the AStyle Release Lib Configuration using the AStyle project file
...
if not exist astylebuild-lib.bat (
set called=yes
set astyle=..\..\%astyle%
set bindir=..\..\%bindir%
)
...
%windir%\microsoft.net\framework\v3.5\msbuild  /nologo  "/property:Configuration=Release Lib"  "%astyle%\AStyle.sln"
...
xcopy "%astyle%\bin\AStyleLib.lib"  "%bindir%\"  /y /q
if %ERRORLEVEL% NEQ 0  set errors=yes
)
